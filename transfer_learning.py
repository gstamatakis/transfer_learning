    ...
    with tf.Session(graph=graph, config=config) as sess:
        ...
        for i in range(FLAGS.how_many_training_steps):
            try:
                ...
                if has_distoreted_images:
                    (train_bottlenecks, train_labels) = get_random_distorted_bottlenecks(sess, image_lists, FLAGS.train_batch_size, 'training', FLAGS.image_dir, distorted_jpeg_data_tensor, distorted_image_tensor, resized_image_tensor, bottleneck_tensor)
                else:
                    (train_bottlenecks, train_labels, _) = get_random_cached_bottlenecks(sess, image_lists, FLAGS.train_batch_size, 'training', FLAGS.bottleneck_dir, FLAGS.image_dir, jpeg_data_tensor,decoded_image_tensor, resized_image_tensor, bottleneck_tensor, FLAGS.tfhub_module)
                ...
                if i % FLAGS.eval_step_interval == 0 or i + 1 == FLAGS.how_many_training_steps:
                    ...
                    validation_bottlenecks, validation_labels, _ = get_random_cached_bottlenecks(sess, image_lists, FLAGS.validation_batch_size, 'validation', FLAGS.bottleneck_dir, FLAGS.image_dir, jpeg_data_tensor, decoded_image_tensor, resized_image_tensor, bottleneck_tensor, FLAGS.tfhub_module)
